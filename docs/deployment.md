...
| `workpro-bridge` | WorkPro/Wechaty Bridge（JuziBot puppet service，企业微信扫码登录） |
...
3. 如需启动 WorkPro Bridge，再补齐：`WECHATY_PUPPET_SERVICE_TOKEN`、`WECHATY_PUPPET_SERVICE_AUTHORITY=token-service-discovery-test.juzibot.com`、`WORKPRO_STAFF_USERID`；
4. 执行：
...
```bash
docker compose --profile workpro up -d workpro-bridge
docker compose --profile workpro logs -f workpro-bridge
```

> JuziBot 试用环境建议保留 `WECHATY_PUPPET_SERVICE_AUTHORITY=token-service-discovery-test.juzibot.com`；试用 token 同时只允许登录一个企业微信账号，退出后可切换账号。收费与配额以服务商公告为准，不在应用代码中硬编码。
...
8. 如需 Bridge：`docker compose --profile workpro up -d workpro-bridge`；
9. `curl /health` 校验。
