...
- 控制台当前提供「账号链接 / 工具 / AI 知识库（RAG） / 数据统计 / 设置（模型配置、人员管理、团队管理、权限管理）」等入口；节日问候、产品介绍、FAQ 等素材能力已收纳到运维工具下。账号扫码状态与验证码提交继续走现有 Bridge 控制接口，模型配置与组织权限页面在后端接口接入前仅展示结构化占位与空状态，不伪造持久化结果。
